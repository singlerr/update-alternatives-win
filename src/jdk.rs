...
use windows::Win32::UI::Shell::{FOLDERID_Profile, FOLDERID_ProgramFiles, FOLDERID_ProgramFilesX86, SHGetKnownFolderPath, KF_FLAG_DEFAULT};
            ...
            match entry {
                Ok(entry) => {
                    if is_jdk(entry.path()) {
                        versions.push(JDK::new(entry.file_name().into_string().unwrap(), entry.path()));
                    }
                }
                Err(_) => {
                    continue
